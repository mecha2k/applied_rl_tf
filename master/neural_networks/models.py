...
def convolution_layer(
    inputs, dimensions, filters, kernel_size, strides, gain=np.sqrt(2), activation="relu"
):
...
class DeepQNetwork:
    def __init__(
        self, n_units, n_classes, n_filters, stride, kernel, state_size, action_size, learning_rate
    ):
        ...
        self.network1 = convolution_layer(
            inputs=self.input_matrix,
        ...
        self.network1 = tf.layers.batch_normalization(self.network1, training=True, epsilon=1e-5)

        self.network2 = convolution_layer(
            inputs=self.network1,
        ...
        self.network2 = tf.layers.batch_normalization(
            inputs=self.network2, training=True, epsilon=1e-5
        )

        self.network3 = convolution_layer(
            inputs=self.network2,
        ...
        self.network3 = tf.layers.batch_normalization(
            inputs=self.network3, training=True, epsilon=1e-5
        )

        self.network3 = tf.layers.flatten(inputs=self.network3)

        self.output = fully_connected_layer(
            inputs=self.network3, units=self.n_units, activation="elu"
        )

        self.output = fully_connected_layer(inputs=self.output, units=n_classes, activation=None)
    ...
    def __init__(self, session, environment, action_space, n_batches, n_steps, reuse=False):
        ...
        with tf.variable_scope("model", reuse=reuse):
            ...
            layer1 = tf.layers.batch_normalization(
                convolution_layer(
                    inputs=scaled_images, filters=32, kernel_size=8, strides=4, dimensions=3
                )
            )

            layer2 = tf.layers.batch_normalization(
                convolution_layer(
                    inputs=tf.nn.relu(layer1), filters=64, kernel_size=4, strides=2, dimensions=3
                )
            )

            layer3 = tf.layers.batch_normalization(
                convolution_layer(
                    inputs=tf.nn.relu(layer2), filters=64, kernel_size=3, strides=1, dimensions=3
                )
            )

            layer3 = tf.layers.flatten(inputs=layer3)
            output_layer = fully_connected_layer(inputs=layer3, units=512, activation="softmax")
            self.distribution, self.logits = self.distribution_type.pdfromlatent(
                output_layer, init_scale=0.01
            )
...
class A3CModel:
    def __init__(self, s_size, a_size, scope, trainer):
        ...
        with tf.variable_scope(scope):
            ...
            self.input_layer = tf.placeholder(shape=[None, s_size], dtype=tf.float32)

            self.input_layer = tf.reshape(self.input_layer, shape=[-1, 84, 84, 1])

            self.layer1 = tf.layers.batch_normalization(
                convolution_layer(
                    inputs=input_layer, filters=32, kernel_size=8, strides=4, dimensions=3
                )
            )

            self.layer2 = tf.layers.batch_normalization(
                convolution_layer(
                    inputs=tf.nn.relu(layer1), filters=64, kernel_size=4, strides=2, dimensions=3
                )
            )
            ...
            outputs, cell_state, hidden_state = lstm_layer(
                input=hidden, size=s_size, actions=a_size, apply_softmax=False
            )
