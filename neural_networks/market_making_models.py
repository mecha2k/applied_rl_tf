...
class DeepQNetworkMM:
    def __init__(self, n_units, n_classes, state_size, action_size, learning_rate):
        ...
        self.layer1 = fully_connected_layer(
            inputs=self.input_matrix, units=self.n_units, activation="selu"
        )

        self.hidden_layer = fully_connected_layer(
            inputs=self.layer1, units=self.n_units, activation="selu"
        )

        self.output_layer = fully_connected_layer(
            inputs=self.hidden_layer, units=n_classes, activation=None
        )
