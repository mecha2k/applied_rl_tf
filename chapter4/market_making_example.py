...
def exploit_explore(
    session, model, explore_start, explore_stop, decay_rate, decay_step, state, actions
):
...
def train_model(environment, dql=None, pg=None, baseline=None):
    ...
    if baseline:
        ...
        for episode in range(n_episodes):
            ...
            for _ in range(100):
                action = baseline_model(n_actions=n_actions, info=info)
            ...
            print("Episode: {}".format(episode), "Total reward: {}".format(reward_sum))
        ...
        with tf.Session() as sess:
            ...
            for episode in range(n_episodes):
                ...
                while current_step < max_steps:
                    ...
                    Qs_next_state = sess.run(
                        model.predicted_Q,
                        feed_dict={model.input_matrix: next_states, model.actions: actions},
                    )
                    ...
                    error_rate, _ = sess.run(
                        [model.error_rate, model.optimizer],
                        feed_dict={
                            model.input_matrix: states,
            ...
            if episode == n_episodes - 1:
                ...
                market_making(
                    model=model, environment=environment, sess=sess, state=state, dpl=True
                )
        ...
        policy_model, model_predictions = mlp_model.create_policy_model(
...
def market_making(model, environment, sess, state, dpl=None, pg=None):
    ...
    for _ in range(1000):
        ...
        for __ in range(100):
            ...
            if dpl == True:
                Q_matrix = sess.run(
                    model.output_layer, feed_dict={model.input_matrix: state.reshape((1, 8))}
                )
        ...
        print("Episode: {}".format(_), "Total reward: {}".format(total_reward))
        scores.append(total_reward)
